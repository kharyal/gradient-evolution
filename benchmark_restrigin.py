...
def mutate(parent_weights,prev_grad, parent_reward,pool,num_eta = 30,t = 0.00001,lr = 0.1, gamma = 0.3):
    ...
    if np.linalg.norm(grad.T + gamma* prev_grad)/grad.shape[0] < 0.1:
        # print("random")
...
def train(weights, pool, max_iteration = 50):
    ...
    for i in range(max_iteration):
        ...
        for j in range(gen_size//2):
            ret = mutate(weights[:,j],grads[:,j],err[j,0],pool, num_eta=num_eta, lr = lr)
