...
def mutate(parent_weights,parent_reward,env,shape,pool,num_eta = 30,t = 0.001,lr = 0.5):
...
def train(weights, shape, env, pool, max_iteration = 50):
    ...
    for i in range(max_iteration):
        ...
        new_weights = np.array([mutate(weights[:,j],err[j,0],env,shape,pool) for j in range(gen_size//2)])
