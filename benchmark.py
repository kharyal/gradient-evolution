...
def get_reward(weights,env,shape,ep_max_step=200, is_continuous = [False,2]):
    s = env.reset()
    # print(weights.shape)
    ...
    for i in range(ep_max_step):
        # s = s.reshape(1,24)
        a = get_action(params,s,shape,is_continuous = is_continuous)
        a = a.reshape(4,1)
...
def get_action(params,state,shape, is_continuous = [False, 2]):
    # params = params_reshape(weights,shape)
    # x = state[np.newaxis, :]
    x = state
    # print(x.shape)
...
def mutate(parent_weights,parent_reward,env,shape,pool,num_eta = 30,t = 0.001,lr = 0.1, is_continuous = [False, 2]):
    eta = choose_eta((parent_weights.shape[0],num_eta))
    eta_weights = parent_weights[:,None] + t*eta
    jobs = [pool.apply_async(get_reward,(eta_weights[:,k], env, shape), {"is_continuous": is_continuous}) for k in range(num_eta)]
    rewards = np.array([j.get() for j in jobs])
    b = ((rewards - parent_reward)/t)
    b = np.reshape(b, (max(b.shape),1))
...
def train(weights, shape, env, pool, max_iteration = 20, is_continuous = [False, 2]):
    num_eta = 6
    for i in range(max_iteration):
        num_eta = min(num_eta+i, 30)
        jobs = [pool.apply_async(get_reward,(weight.T,env,shape), {"is_continuous": is_continuous}) for weight in weights.T]
        rewards = np.array([j.get() for j in jobs])
        ...
        new_weights = np.array([mutate(weights[:,j],err[j,0],env,shape,pool, num_eta=num_eta, is_continuous = is_continuous) for j in range(weights.shape[1])])
...
def train_random(weights, shape, env, pool, max_iteration = 20, is_continuous = [False, 2]):
    for i in range(max_iteration):
        jobs = [pool.apply_async(get_reward,(weight.T,env,shape), {"is_continuous": is_continuous}) for weight in weights.T]
        rewards = np.array([j.get() for j in jobs])
        err = np.array(sorted(zip(rewards,np.arange(gen_size)),reverse = True))
...
if __name__ == "__main__":
    ...
    train(weights,shape,env,pool, iterations, is_continuous=is_continuous)
    # train_random(weights, shape, env, pool, iterations, is_continuous=is_continuous)