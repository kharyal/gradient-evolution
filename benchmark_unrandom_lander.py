...
from lunar_lander_unrandom import LunarLander as Unrandom_LunarLander
...
def mutate(parent_weights,parent_reward,env,shape,pool,num_eta = 30,t = 0.01,lr = 0.1):
    ...
    if np.linalg.norm(grad)/grad.shape[0] < 0.1:
        print("random")
        grad = np.random.uniform(-1/lr,1/lr, grad.shape)
    else:
        print("")
...
def train(weights, shape, env, pool, max_iteration = 50):
    ...
    for i in range(max_iteration):
        ...
        new_weights = np.array([mutate(weights[:,j],err[j,0],env,shape,pool, num_eta=num_eta, lr = lr) for j in range(gen_size//2)])
        new_weights = np.reshape(new_weights,(weights.shape[0],gen_size//2))
        # print(new_weights.shape)
        weights = np.hstack((weights,new_weights))
        # print(weights.shape)
        # exit()
        if abs(avg_top5 - prev_avg_top5)<0.1:
            lr = min(lr*1.2, 100)
        else:
            lr = 0.05
        print(err)
...
if __name__ == "__main__":
    ...
    # weights = np.loadtxt('/home/tanmay/Downloads/290.csv')
